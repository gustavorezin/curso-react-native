...
public class ScriptRunnerUtil {
	...
	public ScriptRunnerUtil(Connection connection) {
	...
	private void executeFullScript(Reader reader) {
		...
		try {
			...
			String message = "Erro na execucao do script: " + script + ".  Cause: " + e;
	...
	private void executeLineByLine(Reader reader) {
		...
		try {
			...
			String message = "Erro na execucao do comando: " + command + ".  Cause: " + e;
			printlnError(message);
			log.error(message, e);
		}
	}
	...
	private void setAutoCommit() {
		try {
			...
			log.error("Could not set AutoCommit to " + autoCommit + ". Cause: " + t, t);
		}
	}
	...
	private void commitConnection() {
		try {
			...
			log.error("Could not commit transaction. Cause: " + t, t);
		}
	}
	...
	private void checkForMissingLineTerminator(StringBuilder command) {
		if (command != null && command.toString().trim().length() > 0) {
			log.error("Line missing end-of-line terminator (" + delimiter + ") => " + command);
		}
	}
	...
	private void printResults(Statement statement, boolean hasResults) {
		try {
			...
			printlnError("Erro mostrando resultados: " + e.getMessage());
		}
	}
